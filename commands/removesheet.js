const { SlashCommandBuilder, ModalBuilder, TextInputBuilder, TextInputStyle, ActionRowBuilder, InteractionCollector, InteractionType, PermissionFlagsBits } = require('discord.js');
const { doc, getDoc, deleteDoc } = require('firebase/firestore');

module.exports = {
	data: new SlashCommandBuilder()
		.setName('remove')
		.setDescription('Deletes target\'s character sheet.')
		.addUserOption(option =>
			option
				.setName('target')
				.setDescription('The member whose sheet will be deleted.')
				.setRequired(true))
	...
	async execute(interaction) {
		...
		if (docSnap.exists()) {
			...
			const delConfirm = new TextInputBuilder()
				...
				.setLabel('Type "DELETE" to delete the character sheet.')
			...
			collector.on('collect', async (i) => {
				...
				if (answer.toLowerCase() === 'delete') {
					await deleteDoc(doc(i.client.db, 'users', targetId));
					await i.reply({ content: 'Character sheet has been deleted.', ephemeral: true });
					const roles = [
						'1070079703261651036',
						'1070079893871788163',
						'1070079896296112201',
					...
					for (const roleId of roles) {
						interaction.guild.roles.fetch(roleId).then(removeRole => {
							interaction.options.getMember('target').roles.remove(removeRole);
						});
					}
				}
				else {
			...
			interaction.reply({ content: 'User does not have a character sheet.', ephemeral: true });
		}
	},
};